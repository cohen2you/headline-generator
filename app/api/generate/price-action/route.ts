    ...
    return NextResponse.json({ priceActions: validPriceActions });
      ...
      const priceActions = await Promise.all(quotes.map(async (quote) => {
        ...
        if (briefAnalysis) {
          ...
          try {
            // Calculate effective 52-week range (adjust if current price is outside reported range)
            let effectiveYearLow = q.fiftyTwoWeekLow;
            let effectiveYearHigh = q.fiftyTwoWeekHigh;
            if (q.fiftyTwoWeekLow && q.fiftyTwoWeekHigh && q.lastTradePrice) {
              if (q.lastTradePrice < q.fiftyTwoWeekLow) {
                effectiveYearLow = q.lastTradePrice; // Current price is new 52-week low
              }
              if (q.lastTradePrice > q.fiftyTwoWeekHigh) {
                effectiveYearHigh = q.lastTradePrice; // Current price is new 52-week high
              }
            }
